<?php function connexionBdd()
{
	...
	try
    {
        $bdd = new PDO('mysql:host=$server;dbname=$dbname;charset=utf8', $utilisateur, $mdp);
...
function comparaisonDates($date1,$date2)//regarde si $date1 est avant (false) ou après (true) $date2
{
...
function comparaisonHeures($heure1,$heure2)//regarde si $h1 est avant ou égal (false) ou après (true) $h2
{
    $heures1 = explode(":", $heure1);
    $heures2 = explode(":", $heure2);
...
function creationTrajet($etapes)
{
    $etape=explode("/", $etapes);
    $trajet=ucfirst($etape[0]);
    $i=1;
    while (isset($etape[$i])) 
    {
      $trajet=$trajet."->".ucfirst($etape[$i]);
      $i=$i+1;  
    }
    return $trajet;
}

function creationPlaceDispo($places)
{
...
function creationHeures($heures)
{
